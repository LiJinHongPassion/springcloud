...
import com.cqut.li.springcloud.entity.Dept;
...
public class UserController_provider {
    ...
    public String getUserById(@RequestParam("user_id") String user_id) {
        ...
        if(user_id.equals("111"))
        {
            throw new RuntimeException("该ID："+user_id+"测试回调方法");
        }
        Map<String, Object> result = userService.getUserById(user_id);
        result.put("url", "springcloud-microservicecloud-hystrix-provider-8081");

        return JSON.toJSON(result).toString().replace("\"", "'");
    }

    public String processHystrix_Get(@RequestParam("user_id") String user_id)
    {
        return "回调显示错误信息  user_id:" + user_id + "\t" + new Date();
    }
